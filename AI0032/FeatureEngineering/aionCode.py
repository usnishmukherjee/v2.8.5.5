...
File generation time: 2023-10-20 16:02:05
'''
...
def featureSelector(log):        
    ...
    model_based_feat = []
    estimator = Lasso()
    estimator.fit(df[train_features],df[target_feature])
    rfe = RFE(estimator, n_features_to_select=1, verbose =0 )
    rfe.fit(df[train_features],df[target_feature])
    ranks = MinMaxScaler().fit_transform(-1*np.array([list(map(float, rfe.ranking_))]).T).T[0]
    ranks = list(map(lambda x: round(x,2), ranks))
    for item, rank in zip(df.columns,ranks):
        if rank > 0.30:
            model_based_feat.append(item)
    if target_feature in model_based_feat:
        model_based_feat.remove(target_feature)
    selected_features['modelBased'] = model_based_feat
    log.info(f'Highly Correlated Features : {model_based_feat}')
